...
def build_final_text(
    ...
    text = f"## 💪 **Ваша суперсила**\n"
    text += f"**{superpower}**. Это отличная база для IT, даже если вы пока не писали код.\n"

    text += f"## 📈 **Зона роста**\n"
    text += f"**{growth_zone}**. Но не пугайтесь: у нас учатся с нуля, и преподаватели помогут прокачать всё, чего не хватает.\n"
    
    text += f"## 🛠️ **С нуля до профи**\n"
    text += "Не волнуйтесь, если у вас пока мало опыта. В Южном университете (ИУБиП) мы учим с азов. Главное — желание развиваться и интерес к технологиям. Хардскиллы придут с практикой.\n"
    
    all_low = all(percent < 20 for percent in all_sub_percentages.values())
    if all_low:
        text += f"## ✨ **Вы новичок?** ✨\n"
        text += "Это даже хорошо — мы научим всему с самого начала. У нас нет проходных баллов за опыт, только за желание.\n"
    
    if top1_percentage < 30:
        text += f"## 💫 **Видим ваш потенциал**\n"
        text += f"Мы видим ваш потенциал, даже если сейчас цифры невысокие. Наши программы подходят для старта с нуля.\n"
    
    text += f"## 🌟 **В заключение**\n"
    text += f"Ждём вас в списке первокурсников в новом учебном году! Приходите к нам в колледж или бакалавриат — и вместе мы сделаем из вас востребованного IT-специалиста.\n"
    text += f"___"
