...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:5173",
        "http://localhost:3000",
        "https://applicants-test.ru.tuna.am/",  # Your Tuna frontend URL
        "https://*.tuna.am",
    ],
...
async def calculate_results(request: CalculateRequest):
    """
    POST /calculate
    Принимает ответы, возвращает результат (без сохранения в БД)
    """
    logger.info(f"Calculate request for session: {request.session_id}")
    logger.info(f"Answers received: {len(request.answers)}")
    
    # Преобразуем в формат для calculations.py
    answers_list = []
    for ans in request.answers:
        ...
        logger.info(f"Question {ans.question_id} -> answer {ans.answer_id}")
    
    # Проверяем наличие ответа на фильтр-вопрос (id=0)
    level_answer = next((a for a in answers_list if a["question_id"] == 0), None)
    if not level_answer:
        logger.error("Missing level question (id=0)")
        raise HTTPException(status_code=400, detail="Missing level question (id=0)")
    
    # Проверяем остальные вопросы (1-17)
    required = set(range(1, 18))
    answered = set(a["question_id"] for a in answers_list)
    missing = required - answered
    if missing:
        logger.error(f"Missing answers for questions: {missing}")
        raise HTTPException(status_code=400, detail=f"Missing answers for questions: {missing}")
...
if os.path.exists("dist"):  # Vite build output
